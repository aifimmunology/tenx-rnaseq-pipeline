...
option_list <- list(
  ...
  make_option(opt_str = c("-s", "--in_sum"),
              ...
              metavar = "character"),
  make_option(opt_str = c("-p", "--in_pbm"),
              type = "character",
              default = NULL,
              help = "Input per_barcode_metrics.csv file",
              metavar = "character"),
...
rmd_loc <- file.path(args$out_dir,
                     paste0(args$in_well,
                            "_add_arc_rna_metadata.Rmd"))

file.copy(system.file("rmarkdown/add_arc_rna_metadata.Rmd", package = "H5weaver"),
          rmd_loc,
          overwrite = TRUE)
